...
class AudioStreamer:
    ...
    def __init__(
        ...
        *,
        ...
        if cfg is None:
            dshow_buffer_raw = os.getenv("AUDIO_DSHOW_BUFFER_MS", "20").strip() or "20"
            try:
                dshow_buffer_ms = max(1, int(dshow_buffer_raw))
            except ValueError:
                dshow_buffer_ms = 20
            cfg = AudioRtpConfig(
                ...
                dshow_audio_buffer_ms=dshow_buffer_ms,
            )
        self.cfg = cfg
    ...
    def start(self, target_ip: Optional[str] = None, target_port: Optional[int] = None, **kwargs) -> None:
        ...
        # dshow defaults to a large capture buffer on some Windows drivers
        # (often causing bursty packets every ~0.5s). Keep this small for
        # stable real-time RTP pacing.
        if backend == "dshow":
            cmd.extend(["-audio_buffer_size", str(self.cfg.dshow_audio_buffer_ms)])
